...
export const videoCard = video => {
    ...
    let sdVideo = video_files.find(item => item.quality === 'sd' && item.width < 1000);
    if (!sdVideo) {
        sdVideo = video_files.find(item => item.width < 1000);
    } 
    const { file_type: type, link } = sdVideo;
    ...
    $card.innerHTML = `
        ...
        <div class="card-content">
            <button class="icon-btn small ${favoriteObj.videos[id] ? 'active' : ''}" aria-label="Add to favorite" data-ripple
                data-favorite-btn>
